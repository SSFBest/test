...
var RKApp = {
    ...
    loginCyword:function(){
        ...
        req.send(function(dataResponse){
            if (dataResponse.hadError()) {
                ...
                top.location = "http://appstore.nate.com/Main/View?apps_no=122";
                return;
            } 
    ...
    getFriends:function(num){
        ...
        var params = {};
        var max = 20;
        params[opensocial.DataRequest.PeopleRequestFields.FIRST] = num*max;
        ...
        req.send(function(dataResponse){
            ...
            if(viewerFriends.getTotalSize()>((num+1)*max)){
    ...
    _initNotice:function(){
        var url = RK_BASE_URL + "/notices/";
        loadHtml(url, {}, "rk_notices");
    },
    
    _initTopBar:function(){
        var hc = "<img src='"+RK_MEDIA_URL+"/images/logo_beta.png' style='float:left'/><ul>"; 
        if(_viewer_id == "62119946"){
            _topbar_def = _topbar_def_2;
        }else{
            _topbar_def = _topbar_def_2;
        }
        for (t in _topbar_def) {
            var tobj = _topbar_def[t];
           	...
           	if (tobj.name=="invite")
               	hc += "<li style='margin-top:-19px'>";
            else if (tobj.name=="addkb")
            ...
            if (tobj.name==_currentTab)
                hc += "<img src='"+RK_MEDIA_URL+ tobj.on + "' id='" +tid+ "' style='cursor:pointer' /></li>";
            else
                hc += "<img src='"+RK_MEDIA_URL+ tobj.off + "' id='" +tid+ "' style='cursor:pointer' /></li>";    
        }
        ...
        //jQuery("#rk_banner").html("<a href=\"http://club.cyworld.com/ClubV1/Home.cy/54119321\" target=\"_blank\"><img id =\"bannerimg\" src=\"http://cwfarm.rekoo.com/static/images/banner1.gif\" /></a>");
    ...
    doMyFarm:function() {   //我的牧场
        ...
        if(_uname != null && _uname.length != 0 && _uname !='None'){
            ...
            var swf_url = RK_MEDIA_URL + "/images/sunfarm.swf?v=" + _flashVersion;
            var flashvars = {
    ...
    doNickName: function(){
        ...
        gadgets.io.makeRequest(RK_BASE_URL + '/changenickname/', RKApp.init, params);
    },
    ...
    doForum: function() {
        window.open("http://club.cyworld.com/clubV1/Home.cy/54119321");
    },
    ...
    doQuestion: function(){
        _currentTab = "question";
        RKApp.setTabOn(_currentTab); 
        RKApp.setMainOnLoading(); 

        var url = RK_BASE_URL + "/into_question/";
        loadHtml(url, {}, "rk_main");
        gadgets.window.adjustHeight(2000);
    },
    ...
    doConfig: function() {
        ...
        RKApp.setTabOn(_currentTab); 
        RKApp.setMainOnLoading(); 
        var url = RK_BASE_URL + "/user_settings/";    	
        loadHtml(url, {}, "rk_main");
        gadgets.window.adjustHeight(800);
    },
    ...
    doInvite: function() {
        _currentTab = "invite";
          RKApp.setTabOn(_currentTab);
          RKApp.setMainOnLoading();
          var url = RK_BASE_URL + "/invite_install/";
          loadHtml(url, {}, "rk_main");
          gadgets.window.adjustHeight(800);
          /**
     	  _currentTab = "invite";
        ...
        jQuery("#rk_main").html("<input id='rk_invite_btn' onclick='RKApp._doInvite();' type='button' value='邀请' />");
		  ...
		  if( response.hadError() ){
            ...
            if(confirm(str.replace(/\w{1}$/,"")+"에게 초대 메일을 보냅니다,확인합니까?"))   {  
              setCookie('loginFeedSent', 'true', 60 * 60 * 24*2);
               var url = RK_BASE_URL + "/install_callback/";
    ...
    setMainOnLoading: function() {
        var hc = "<div height='400' style='margin-top:35px;margin-left:auto;margin-right:auto;text-align:center'>" +
            "<img src='"+RK_MEDIA_URL+"/images/loading.gif' /></div>"
    ...
    setTabOn: function(tabName) {   //设置tab的图片为on状态
        for (t in _topbar_def) {
            ...
            if (tobj.name == tabName)
                jQuery("#"+tid).attr("src", RK_MEDIA_URL + tobj.on);
            else
                jQuery("#"+tid).attr("src", RK_MEDIA_URL + tobj.off); 
        }        
    }
...
var _topbar_def_1 = [
    ...
    */
    ...
    {
        name : "forum",
        on  : "/images/discussion.png",
        off : "/images/discussion.png",
        click : RKApp.doForum
    },
    ...
    {
        name : "invite",
        on  : "/images/invite3.png",
        off : "/images/invite3.png",
    ...
    /*{
        name : "setting",
        on  : "/images/settings2.png",
        off : "/images/settings1.png",
        //click : function(){alert("setting");}
        click : RKApp.doConfig
    },*/
...
var _topbar_def_2 = [
    ...
    {
        name : "question",
        on : "/images/question1.png",
        off : "/images/question2.png",
        click :RKApp.doQuestion
    },
    {
        name : "nickename",
        on : "/images/nickname1.png",
        off : "/images/nickname2.png",
        click : RKApp.doNickName
    }             
];
RKApp.init();
